import { validateText, validateDate, validateCountry, validatePayment, validatePicture } from './formValidation';
...
describe('validatePayment', () => {
    it('should return true if either the first or second radio button is checked', () => {
        ...
        expect(validatePayment(firstRadio as React.RefObject<HTMLInputElement>, secondRadio as React.RefObject<HTMLInputElement>)).toBe(true);
    });

    it('should return false if both the first and second radio buttons are unchecked', () => {
        const firstRadio = { current: { checked: false } };
        const secondRadio = { current: { checked: false } };
        expect(validatePayment(firstRadio as React.RefObject<HTMLInputElement>, secondRadio as React.RefObject<HTMLInputElement>)).toBe(false);
    });
});
