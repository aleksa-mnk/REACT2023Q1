...
describe('CardList', () => {
  it('renders all cards', () => {
    const { getByTestId } = render(React.createElement(CardList, { searchString: '' }));
    const cardList = getByTestId('card-list');
    expect(cardList.childElementCount).toBe(2);
  });
  it('filters cards based on search string', () => {
    const { getByTestId } = render(React.createElement(CardList, { searchString: 'Test Card 1' }));
    const cardList = getByTestId('card-list');
  ...
  it('displays a message when no cards are found', () => {
    ...
    const { getByText } = render(React.createElement(CardList, { searchString: searchString }));
    const message = getByText('No cards found');
    expect(message).toBeInTheDocument();
  });
  it('displays a message when no cards are present in the list', () => {
    const searchString = 'Some non-existent card';
    const { queryByText } = render(React.createElement(CardList, { searchString: searchString }));
